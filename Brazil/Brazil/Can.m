...
- (id)initWithParentNode:(CCNode *)parentNode {
    ...
    if (self = [super init]) {
        ...
        frame = [frameCache spriteFrameByName:PNG_CAN_RIGHT_1];
        right1Sprite = [CCSprite spriteWithSpriteFrame:frame];
        right1Sprite.anchorPoint = ccp(0, 0);
        right1Sprite.position = _can1Sprite.position;
        right1Sprite.visible = NO;
        
        frame = [frameCache spriteFrameByName:PNG_CAN_WRONG_1];
        wrong1Sprite = [CCSprite spriteWithSpriteFrame:frame];
        ...
        frame = [frameCache spriteFrameByName:PNG_CAN_RIGHT_2];
        right2Sprite = [CCSprite spriteWithSpriteFrame:frame];
        right2Sprite.anchorPoint = ccp(0, 0);
        right2Sprite.position = _can2Sprite.position;
        right2Sprite.visible = NO;
        
        frame = [frameCache spriteFrameByName:PNG_CAN_WRONG_2];
        wrong2Sprite = [CCSprite spriteWithSpriteFrame:frame];
        ...
        frame = [frameCache spriteFrameByName:PNG_CAN_RIGHT_3];
        right3Sprite = [CCSprite spriteWithSpriteFrame:frame];
        right3Sprite.anchorPoint = ccp(0, 0);
        right3Sprite.position = _can3Sprite.position;
        right3Sprite.visible = NO;
        
        frame = [frameCache spriteFrameByName:PNG_CAN_WRONG_3];
        wrong3Sprite = [CCSprite spriteWithSpriteFrame:frame];
...
//    CCSequence *seq = [CCSequence acti];
    ...
    switch (rubbishTag) {
        ...
        case RubbishNewspaperTag:
            ...
            CCCallBlock *rightSound = [CCCallBlock actionWithBlock:^{
                [[SimpleAudioEngine sharedEngine] playEffect:EFFECT_RUBBISH_RIGHT];
            }];
            right = [CCSequence actions:rightSound, showRight, colorGreen, delay, colorNormal, rubbishVisible, hideRight, nil];
        ...
        default:
            ...
            CCCallBlock *wrongSound = [CCCallBlock actionWithBlock:^{
                [[SimpleAudioEngine sharedEngine] playEffect:EFFECT_RUBBISH_WRONG];
            }];
            
            wrong = [CCSequence actions:wrongSound, showWrong, colorRed, delay, colorNormal, rubbishVisible, hideWrong, nil];
...
- (void)setCanHarmResultByRubbishTag:(int)rubbishTag {
    ...
    switch (rubbishTag) {
        ...
        case RubbishBottleTag:
            ...
            CCCallBlock *showRight = [CCCallBlock actionWithBlock:^{
                [right2Sprite runAction:[CCShow action]];
            }];
            
            CCCallBlock *hideRight = [CCCallBlock actionWithBlock:^{
                [right2Sprite runAction:[CCHide action]];
            }];
        ...
        default:
            ...
            CCCallBlock *showWrong = [CCCallBlock actionWithBlock:^{
                [wrong2Sprite runAction:[CCShow action]];
            }];
            
            CCCallBlock *hideWrong = [CCCallBlock actionWithBlock:^{
                [wrong2Sprite runAction:[CCHide action]];
            }];
...
- (void)setCanKitchenResultByRubbishTag:(int)rubbishTag {
    ...
    switch (rubbishTag) {
        ...
        case RubbishCokeTag:
            ...
            CCCallBlock *showRight = [CCCallBlock actionWithBlock:^{
                [right3Sprite runAction:[CCShow action]];
            }];
            
            CCCallBlock *hideRight = [CCCallBlock actionWithBlock:^{
                [right3Sprite runAction:[CCHide action]];
            }];
        ...
        default:
            ...
            CCCallBlock *showWrong = [CCCallBlock actionWithBlock:^{
                [wrong3Sprite runAction:[CCShow action]];
            }];
            
            CCCallBlock *hideWrong = [CCCallBlock actionWithBlock:^{
                [wrong3Sprite runAction:[CCHide action]];
            }];
...
- (void)saylikeRubbish:(int)canTag {
    ...
    BOOL isEn = [[[RubbishScene sharedRubbishScene] navBar] isEnglish];
    NSString *name = nil;
    switch (canTag) {
        case CanRecycleTag:
            ...
            if (isEn) {
                name = SOUND_RECYCLABLE_EN;
            } else {
                name = SOUND_RECYCLABLE_CN;
            }
        ...
        case CanHarmTag:
            ...
            if (isEn) {
                name = SOUND_OTHERWASTE_EN;
            } else {
                name = SOUND_OTHERWASTE_CN;
            }
            break;
        ...
        case CanKitchenTag:
            ...
            if (isEn) {
                name = SOUND_KITCHENWASTE_EN;
            } else {
                name = SOUND_KITCHENWASTE_CN;
            }
            break;
